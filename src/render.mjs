...
function brandsSection() {
  return `<section class="brands-section" aria-labelledby="brands-title"><div class="container brands-layout"><div class="brands-copy">${eyebrow("Marcas que nos respaldan")}<h2 id="brands-title">Fabricantes líderes<br>para su operación.</h2><p>Trabajamos con marcas reconocidas para protección industrial, dotaciones y proyectos especiales.</p></div><div class="brand-strip">${brands.map(b=>`<div class="brand-logo"><img src="${b.image}" width="300" height="300" loading="lazy" alt="${esc(b.name)}"></div>`).join("")}</div></div></section>`;
}
