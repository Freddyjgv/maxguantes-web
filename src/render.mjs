...
export function renderProduct(product, products) {
  ...
  const variantSelector = product.structuredVariants?.length ? `<label class="product-variant-picker"><span>Presentación</span><select data-product-variant>${product.structuredVariants.map(variant => { const details = [variant.attributes?.talla, variant.attributes?.color, variant.label].filter(Boolean).join(" · ") || variant.public_code; return `<option value="${esc(variant.public_code)}">${esc(details)}</option>`; }).join("")}</select></label>` : "";
