...
export function renderProduct(product, products) {
  ...
  const variantSelector = product.structuredVariants?.length
    ? `<label class="product-variant-picker"><span>Seleccione talla, color o presentación</span><select data-product-variant>${product.structuredVariants.map(variant => { const details = variant.label || [variant.attributes?.talla, variant.attributes?.color].filter(Boolean).join(" · ") || variant.public_code; return `<option value="${esc(variant.public_code)}" data-variant-id="${esc(variant.id)}" data-variant-label="${esc(details)}" data-variant-attributes="${esc(JSON.stringify(variant.attributes || {}))}" data-dolibarr-product-id="${esc(variant.dolibarr_product_id ?? product.dolibarrProductId ?? "")}" data-dolibarr-ref="${esc(variant.dolibarr_ref || product.dolibarrRef || "")}">${esc(details)}</option>`; }).join("")}</select><small>Agregue cada talla o presentación como una línea independiente.</small></label>`
    : `<label class="product-variant-picker product-variant-custom"><span>Talla, color o presentación <em>(opcional)</em></span><input type="text" maxlength="120" data-product-custom-variant placeholder="Ej.: Talla 40 · color negro"><small>Si este producto no tiene variantes creadas, indique aquí el detalle que debemos cotizar.</small></label>`;
