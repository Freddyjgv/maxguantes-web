...
function categoryCards(products) {
  return `<div class="category-grid">${categories.map((c, index) => {
    const representative = products.find(product => product.category === c.slug);
    return `<a class="category-card" href="/categorias/${c.slug}/"><div class="category-card-media"><img src="${esc(representative?.image || "/assets/product-placeholder.svg")}" width="560" height="360" loading="lazy" alt=""><span class="category-index">${String(index + 1).padStart(2,"0")} / ${c.code}</span><span class="category-symbol">${c.code}</span></div><div class="category-card-copy"><h3>${c.name}</h3><p>${c.description}</p><span class="card-link">Explorar categoría →</span></div></a>`;
  }).join("")}</div>`;
}

function distributionBanner() {
  return `<section class="distribution-banner" aria-label="Marcas de distribución"><div class="container distribution-inner"><div class="distribution-copy"><span class="distribution-kicker">Distribución especializada</span><strong>Distribuidores de Portwest y General Electric PPE</strong><p>Acceso a líneas reconocidas de protección industrial, documentación técnica y atención comercial para empresas y proyectos.</p></div><div class="distribution-logos"><img src="/assets/brands/portwest.png" width="150" height="150" alt="Portwest"><span aria-hidden="true">+</span><img src="/assets/brands/general-electric-ppe.png" width="150" height="150" alt="General Electric PPE"></div><a class="text-link" href="/productos/">Ver productos →</a></div></section>`;
}
...
export function renderHome(products) {
  ...
  const content = `
    <section class="hero">
      ...
      <div class="container hero-content"><div class="hero-copy">${eyebrow("Suministro de EPP certificado · Panamá", true)}<h1 class="display">Protección certificada.<br><span>Respuesta inmediata.</span></h1><p class="hero-lead">Equipos de seguridad industrial para construcción, manufactura, metalmecánica, logística y operaciones de alto riesgo. Le ayudamos a seleccionar y cotizar la protección correcta.</p><div class="button-row"><a class="btn btn-primary" href="/productos/">Explorar productos →</a><button class="btn btn-outline" type="button" data-open-quote>Solicitar cotización</button></div></div></div>
      <div class="hero-proof"><div class="container proof-grid"><div class="proof-item"><strong>Desde 2015</strong><span>Experiencia en suministro industrial</span></div><div class="proof-item"><strong>Asesoría técnica</strong><span>Selección según riesgo y aplicación</span></div><div class="proof-item"><strong>Catálogo ampliado</strong><span>Alta rotación y pedidos especiales</span></div><div class="proof-item"><strong>Respuesta directa</strong><span>Disponibilidad confirmada por nuestro equipo</span></div></div></div>
    </section>
    ${distributionBanner()}
    <section class="section"><div class="container"><div class="section-head"><div>${eyebrow("Atención acorde a su empresa")}<h2 class="section-title">Protección organizada<br>por necesidad.</h2><p class="section-intro">Navegue por categoría, riesgo o aplicación. Cada ficha está diseñada para facilitar la revisión técnica y la solicitud de cotización.</p></div><a class="text-link" href="/productos/">Ver catálogo completo</a></div>${categoryCards(products)}</div></section>
    ...
    <section class="section"><div class="container"><div class="section-head"><div>${eyebrow("Selección técnica")}<h2 class="section-title">Productos destacados.</h2><p class="section-intro">Referencias de alta rotación y soluciones especializadas disponibles para cotización. Precio y disponibilidad se confirman para cada solicitud.</p></div><a class="text-link" href="/productos/">Ver productos</a></div>${featured.length ? `<div class="product-grid">${featured.map(productCard).join("")}</div>` : emptyCatalog()}</div></section>
    ...
    <section class="section story-preview"><div class="container split-layout"><div class="image-frame"><img src="/assets/about-team.webp" width="1800" height="1200" loading="lazy" alt="Equipo técnico revisando una selección de equipos de protección personal"></div><div class="split-copy">${eyebrow("Empresa panameña")}<h2 class="section-title">Una década resolviendo necesidades de protección.</h2><p>Maxguantes nació en Panamá en 2015 para responder con rapidez y eficiencia a empresas que necesitan más que una lista de productos: asesoría, documentación técnica y acompañamiento en la selección.</p><p>Atendemos requerimientos de construcción, manufactura, metalmecánica, transporte, logística y otras actividades con necesidades específicas de seguridad industrial.</p><div class="metric-row"><div><strong>2015</strong><span>Año de fundación</span></div><div><strong>B2B</strong><span>Atención especializada</span></div><div><strong>LATAM</strong><span>Alcance comercial</span></div></div><a class="text-link" href="/nosotros/">Conozca nuestra historia</a></div></div></section>
    <section class="section-sm brands-section"><div class="container"><div class="section-head compact"><div>${eyebrow("Marcas que distribuimos")}<h2>Respaldo de fabricantes reconocidos.</h2></div><p>Seleccionamos referencias certificadas y confirmamos documentación, precio y disponibilidad para cada requerimiento.</p></div><div class="brand-strip">${brands.map(brand=>`<div class="brand-logo"><img src="${brand.logo}" width="150" height="150" loading="lazy" alt="${brand.name}"></div>`).join("")}</div></div></section>`;
