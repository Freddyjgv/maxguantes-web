...
function productCard(product) {
  ...
  return `<article class="product-card" data-product-card data-name="${esc(product.name.toLowerCase())}" data-sku="${esc(product.sku.toLowerCase())}" data-categories="${esc((product.categories || [product.category]).join(" "))}" data-brand="${esc((product.brand || "").toLowerCase())}" data-brand-slug="${anchorId(product.brand || "maxguantes")}" data-certifications="${esc(standards.join(" ").toLowerCase())}">
    ...
    <div class="product-body"><span class="product-code">${esc(product.sku)} · ${esc(product.brand || "Maxguantes")}</span><h3><a href="/productos/${product.slug}/">${esc(product.name)}</a></h3><p>${esc(product.summary)}</p>${certifications}<div class="product-purchase"><label class="quantity-picker"><span class="sr-only">Cantidad de ${esc(product.name)}</span><button type="button" data-quantity-minus aria-label="Reducir cantidad">−</button><input type="number" min="1" max="9999" value="1" inputmode="numeric" data-product-quantity aria-label="Cantidad de ${esc(product.name)}"><button type="button" data-quantity-plus aria-label="Aumentar cantidad">+</button></label><button class="btn btn-dark add-quote" type="button" data-code="${esc(product.sku)}" data-name="${esc(product.shortName || product.name)}" data-image="${esc(product.image)}">Agregar</button><a class="icon-button" href="/productos/${product.slug}/" aria-label="Ver ${esc(product.name)}">→</a></div></div>
  </article>`;
}
...
export function renderProduct(product, products) {
  ...
  const content = `<section class="product-detail"><div class="container"><nav class="breadcrumb dark" aria-label="Migas de pan"><a href="/">Inicio</a><span>/</span><a href="/productos/">Productos</a><span>/</span><a href="/categorias/${product.category}/">${esc(product.categoryName)}</a><span>/</span><span aria-current="page">${esc(product.sku)}</span></nav><div class="product-detail-grid"><div class="product-gallery">${product.specialOrder ? '<span class="product-status">Pedido especial</span>' : ""}<img src="${esc(product.image)}" width="900" height="900" fetchpriority="high" alt="${esc(product.name)}"></div><div class="product-summary">${eyebrow(`${esc(product.brand)} · ${esc(product.sku)}`, true)}<h1>${esc(product.name)}</h1><p class="product-lead">${esc(product.summary)}</p><div class="notice"><strong>Cotización personalizada</strong><span>No mostramos precio ni inventario. Confirmamos ambos según el cliente, cantidad y fecha requerida.</span></div><div class="detail-add-row"><label class="quantity-picker quantity-picker-large"><span class="sr-only">Cantidad de ${esc(product.name)}</span><button type="button" data-quantity-minus aria-label="Reducir cantidad">−</button><input type="number" min="1" max="9999" value="1" inputmode="numeric" data-product-quantity aria-label="Cantidad de ${esc(product.name)}"><button type="button" data-quantity-plus aria-label="Aumentar cantidad">+</button></label><button class="btn btn-primary btn-large add-quote" type="button" data-code="${esc(product.sku)}" data-name="${esc(product.shortName || product.name)}" data-image="${esc(product.image)}">Agregar a solicitud</button></div><a class="btn btn-outline" href="${whatsappUrl(`Hola Maxguantes, deseo cotizar el producto ${product.sku} - ${product.name}.`)}" target="_blank" rel="noopener">Consultar por WhatsApp</a></div></div></div></section>
