...
function productCard(product) {
  ...
  const variantPicker = product.structuredVariants?.length ? `<label class="card-variant-picker"><span>Elija una variante *</span><select data-product-variant required aria-label="Seleccione una variante de ${esc(product.name)}">${variantOptions(product)}</select></label>` : "";
...
function variantOptions(product) {
  return `<option value="" selected disabled>Seleccione una opción…</option>` + product.structuredVariants.map(variant => {
    const details = variant.label || Object.values(variant.attributes || {}).filter(Boolean).join(" · ") || variant.public_code;
...
export function renderProduct(product, products) {
  ...
  const variantSelector = product.structuredVariants?.length
    ? `<label class="product-variant-picker"><span>Seleccione talla, color o presentación *</span><select data-product-variant required>${variantOptions(product)}</select><small>La variante es obligatoria. Para solicitar dos tallas, agregue cada una por separado.</small></label>`
    : "";
